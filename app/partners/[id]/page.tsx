...
import { Card, Badge, PageHeader, ScoreBar, SectionLabel, Divider } from '@/components/ui';
import { scorePartner } from '@/lib/scoring';

export default function PartnerDetailPage({ params }: { params: { id: string } }) {
  const db = getDb();
  const partner = db.prepare('SELECT * FROM partners WHERE id = ?').get(params.id) as any;
  if (!partner) return notFound();

  const deals = db.prepare('SELECT * FROM deals WHERE partner_id = ? ORDER BY created_at DESC').all(params.id) as any[];
  const outreachList = db.prepare('SELECT * FROM outreach WHERE partner_id = ? ORDER BY created_at DESC').all(params.id) as any[];
      ...
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '8px' }}>
            <Link href={`/deal-builder?partner=${params.id}`} style={{
            ...
            <Link href={`/outreach?partner=${params.id}`} style={{
