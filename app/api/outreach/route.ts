...
export async function POST(req: NextRequest) {
  try {
    ...
    const output = generateOutreach({
      ...
      archetype, staged_rollout, style,
    });

    const id = uuidv4();
    const saved = {
      id, partner_id, prospect_name, prospect_title,
      company: company || partner.name, style,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const results = Object.values(store.outreach)
